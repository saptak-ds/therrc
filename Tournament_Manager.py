# Tournament_Manager.py (Full Code, Corrected for Fixture Ordering)
...
class DatabaseManager:
    ...
    def create_tables(self):
        ...
        self.conn.commit()
    # Add this method inside the DatabaseManager class in Tournament_Manager.py

    def recalculate_team_stats(self, team_id):
        """Recalculates all stats for a team from scratch and updates the database."""
        # First, reset the team's stats to zero
        self.cursor.execute("""
            UPDATE teams 
            SET matches_played=0, wins=0, draws=0, losses=0, goals_for=0, goals_against=0 
            WHERE team_id = ?
        """, (team_id,))
        
        # Get all played fixtures for this team
        self.cursor.execute("""
            SELECT * FROM fixtures 
            WHERE (team1_id = ? OR team2_id = ?) AND status = 'Played' 
              AND (stage LIKE 'Group%' OR stage = 'League')
        """, (team_id, team_id))
        
        played_fixtures = self.cursor.fetchall()
        
        # Loop through each match and calculate stats
        ...
        # Finally, update the database with the fresh, correct stats
        self.cursor.execute("""
            UPDATE teams 
            SET matches_played=?, wins=?, draws=?, losses=?, goals_for=?, goals_against=?
            WHERE team_id = ?
        """, (stats['mp'], stats['w'], stats['d'], stats['l'], stats['gf'], stats['ga'], team_id))
    ...
    def get_all_fixtures(self, tournament_id):
        # <<< BUG FIX IS HERE >>>
        # Changed the ORDER BY clause to sort by match number only.
...
class Match:
    ...
    @property
    def team1_name(self):
        return self.team1.name if self.team1 else self.p1
    
    @property
    def team2_name(self):
        return self.team2.name if self.team2 else self.p2

    def update_stats(self, new_t1_goals, new_t2_goals, pso_t1=None, pso_t2=None):
        # 1. Update the goals for this specific match in the database
        self.team1_goals = new_t1_goals
        self.team2_goals = new_t2_goals
        self.team1_pso = pso_t1
        self.team2_pso = pso_t2
        self.status = 'Played'
        self.db.update_fixture_result(self)
        
        # 2. Trigger a full recalculation for both teams involved
        # This is only necessary for group/league stage matches
...
class TournamentApp:
    ...
    def generate_fixtures(self, tournament_id, settings, teams_by_group, num_legs=1):
        ...
        placeholders = {}
        if is_league_mode:
            placeholders['A'] = [f"{i}{self._get_ordinal_suffix(i)} Place" for i in range(1, qualifiers_per_group + 1)]
        else:
            placeholders = {g: [f"{g}{i}" for i in range(1, qualifiers_per_group + 1)] for g in group_names}
        if knockout_mode == '3':
            qf_pairings = []
            if is_league_mode: qf_pairings = [(placeholders['A'][0], placeholders['A'][7]), (placeholders['A'][3], placeholders['A'][4]), (placeholders['A'][2], placeholders['A'][5]), (placeholders['A'][1], placeholders['A'][6])]
            elif len(group_names) == 2: qf_pairings = [(placeholders['A'][0], placeholders['B'][3]), (placeholders['A'][2], placeholders['B'][1]), (placeholders['B'][0], placeholders['A'][3]), (placeholders['B'][2], placeholders['A'][1])]
            elif len(group_names) == 4: qf_pairings = [(placeholders['A'][0], placeholders['B'][1]), (placeholders['C'][0], placeholders['D'][1]), (placeholders['B'][0], placeholders['A'][1]), (placeholders['D'][0], placeholders['C'][1])]
        ...
        if knockout_mode in ['2', '3']:
            sf_pairings = []
            if knockout_mode == '2':
                if is_league_mode: sf_pairings = [(placeholders['A'][0], placeholders['A'][3]), (placeholders['A'][1], placeholders['A'][2])]
                elif len(group_names) == 2: sf_pairings = [(placeholders['A'][0], placeholders['B'][1]), (placeholders['B'][0], placeholders['A'][1])]
                elif len(group_names) == 4: sf_pairings = [(placeholders['A'][0], placeholders['B'][0]), (placeholders['C'][0], placeholders['D'][0])]
            else: sf_pairings = [("Winner Quarter-Final 1", "Winner Quarter-Final 4"), ("Winner Quarter-Final 2", "Winner Quarter-Final 3")]
        ...
        if knockout_mode in ['1', '2', '3']:
            fnl_pairings = []
            if knockout_mode == '1':
                if len(group_names) >= 2: fnl_pairings = [(placeholders['A'][0], placeholders['B'][0])]
                elif len(group_names) == 1: fnl_pairings = [(placeholders['A'][0], placeholders['A'][1])]
            else: fnl_pairings = [("Winner Semi-Final 1", "Winner Semi-Final 2")]
            for i, (p1, p2) in enumerate(fnl_pairings): self.db.add_fixture(tournament_id, None, None, p1, p2, "Final", i + 1)
    ...
    def _promote_group_winners(self, tournament_id, all_fixtures, is_league_mode, qualifiers_per_group):
        ...
        for group in group_names:
            ...
            for i in range(qualifiers_per_group):
                if i < len(standings):
                    rank, team_to_promote = i + 1, standings[i]
                    placeholder = f"{rank}{self._get_ordinal_suffix(rank)} Place" if is_league_mode else f"{group}{rank}"
    ...
    def _promote_knockout_winners(self, all_fixtures, completed_stage):
        ...
        for match in completed_fixtures:
            winner = match.team1 if (match.team1_pso > match.team2_pso if match.team1_pso is not None else match.team1_goals > match.team2_goals) else match.team2
